...
Deno.serve(async (req) => {
  ...
  if (
    !["sales", "media"].includes(body.workspace ?? "") ||
    !allowedTools.has(body.tool ?? "")
  )
    return json({ error: "Invalid assistant request." }, 400);
  const prompt = String(body.prompt ?? "").slice(0, 4000);
  const admin = adminClient();
  const context =
    body.workspace === "sales"
      ? await admin
          .from("sales_calls")
          .select(
      ...
      : await admin
          ...
          .select(
            "campaign_name,status,spend,impressions,clicks,leads,purchases,revenue,date_start,date_stop",
          )
          .eq("user_id", user.id)
  ...
  if (workspace === "sales") {
    ...
    const objection =
      rows
        ...
        .sort(
          (a, b) =>
            rows.filter((r) => r.primary_objection === b).length -
            rows.filter((r) => r.primary_objection === a).length,
        )[0] ?? "not enough data";
