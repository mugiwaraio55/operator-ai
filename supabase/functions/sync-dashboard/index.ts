...
const actionCount = (
  ...
  (actions ?? [])
    .filter((row) => names.includes(row.action_type ?? ""))
...
Deno.serve(async (req) => {
  ...
  const body = (await req.json().catch(() => ({}))) as {
    ...
    testClickUp?: boolean;
    setClickUpListId?: string;
  };
  const admin = adminClient();

  if (body.setClickUpListId) {
    const selected = await selectClickUpList(
      admin,
      user.id,
      body.setClickUpListId,
    );
    return selected.ok ? json(selected) : json({ error: selected.error }, 409);
  }
  if (body.testClickUp) {
    const delivered = await createClickUpBrief(
      admin,
      user.id,
      [],
      "Operator AI",
    ...
    return delivered.ok
      ? json(delivered)
      : json({ error: delivered.error }, 409);
  }
  ...
  if (!connection?.account_id)
    return json({ error: "Connect Meta Ads first." }, 409);
  const { data: token, error: tokenError } = await admin.rpc(
    "get_integration_secret",
    { p_user: user.id, p_provider: "meta" },
  );
  if (tokenError || !token)
    return json({ error: "The Meta connection needs to be refreshed." }, 409);

  const insightsUrl = new URL(
    `https://graph.facebook.com/${graphVersion}/act_${connection.account_id}/insights`,
  );
  ...
  const campaignsUrl = new URL(
    `https://graph.facebook.com/${graphVersion}/act_${connection.account_id}/campaigns`,
  );
    ...
    return json(
      {
        error:
          ...
          "Meta insights could not be loaded.",
      },
      502,
    );
  const statusById = new Map(
    (campaignsPayload.data ?? []).map((row) => [
      row.id,
      row.status ?? "UNKNOWN",
    ]),
  );
  const currency =
    typeof connection.metadata?.currency === "string"
  ...
  const rows = (insightsPayload.data ?? []).map((row) => {
    ...
    const values = Array.isArray(row.action_values)
      ? (row.action_values as { action_type?: string; value?: string }[])
      : [];
    return {
      user_id: user.id,
      ...
      leads: actionCount(actions, ["lead", "offsite_conversion.fb_pixel_lead"]),
      purchases: actionCount(actions, [
        "purchase",
        "offsite_conversion.fb_pixel_purchase",
      ]),
      revenue: actionCount(values, [
        "purchase",
        "offsite_conversion.fb_pixel_purchase",
      ]),
      currency,
  ...
  if (rows.length) {
    const { error } = await admin
      ...
      .upsert(rows, {
        onConflict:
          "user_id,ad_account_id,meta_campaign_id,date_start,date_stop",
      });
...
async function selectClickUpList(
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(normalizedId))
    return { ok: false, error: "Enter a valid ClickUp List ID." };
  const { data: token } = await admin.rpc("get_integration_secret", {
    p_user: userId,
    p_provider: "clickup",
  });
  if (!token)
    return { ok: false, error: "Connect ClickUp before choosing a List." };
  const response = await fetch(
    `https://api.clickup.com/api/v2/list/${encodeURIComponent(normalizedId)}`,
  ...
  if (!response.ok || !payload.id)
    return {
      ok: false,
      error:
        "ClickUp could not access that List. Check the List ID and Workspace authorization.",
    };
  const { data: connection } = await admin
    .from("integration_connections")
    .select("metadata")
  ...
  const metadata = {
    ...
    list_name: payload.name ?? `List ${payload.id}`,
    space_id: payload.space?.id ?? null,
    space_name: payload.space?.name ?? null,
  };
  const { error } = await admin
    .from("integration_connections")
    .update({ metadata, refreshed_at: new Date().toISOString() })
    .eq("user_id", userId)
    .eq("provider", "clickup");
  return error
    ? { ok: false, error: error.message }
    : { ok: true, listId: String(payload.id), listName: metadata.list_name };
}

async function createClickUpBrief(
  admin: ReturnType<typeof adminClient>,
  userId: string,
  ...
  if (!token)
    return { ok: false, error: "Connect ClickUp before creating a brief." };
  const listId =
  ...
  if (!listId)
    return {
      ok: false,
      error: "Choose a ClickUp List before creating a brief.",
    };
  ...
  const name = isTest
    ? "Operator AI - ClickUp connection test"
    : `Operator AI - Media Buyer brief - ${new Date().toISOString().slice(0, 10)}`;
  ...
  await admin
    .from("clickup_task_deliveries")
    .insert({
      user_id: userId,
      list_id: listId,
      task_kind: isTest ? "test" : "media_brief",
